...
class Pizzakit
{
	...
	public static function payment_query_handler($data)
	{
		...
		$query = $wpdb->prepare('SELECT payment_status FROM ' . $table . ' WHERE id = %d', $data->get_url_params()['id']);
			...
			if (Pizzakit::validate_callback($order_id, $req_uuid)) {
				if ($resp = Pizzakit::verify_swish_payment($req_uuid)) {
					...
					if ($resp_json['status'] == "PAID") {
						global $wpdb;
						$table = $wpdb->prefix . 'orders';
						$res = $wpdb->update($table, array('payment_status' => $resp_json['status']), array('id' => $_data->get_url_params()['id']), array('%s'), array('%d'));
						if ($res == false) {
							trigger_error("Pizzakit: error creating entry in" . $wpdb->prefix . "orders");
						}
					}
				}
			}
	...
	private static function create_payment($order)
	{
		...
		if ($order_total < 1) {
			global $wpdb;
			$table = $wpdb->prefix . 'orders';
			$data = array('id' => $order_id, 'uuid' => '-2', 'payment_status' => 'INVALID_TOTAL');
			$format = array('%d', '%s', '%s');
		...
		if ($res['response'] !== NULL) {
			global $wpdb;
			$table = $wpdb->prefix . 'orders';
			$data = array('id' => $order_id, 'uuid' => $res['uuid'], 'payment_status' => 'PENDING');
			$format = array('%d', '%s', '%s');
			$wpdb->insert($table, $data, $format);
			return ($order_id);
		}
